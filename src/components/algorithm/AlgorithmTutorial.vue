...
const handleCategoryChange = async (data: { category: string; subCategory?: string; item?: string }) => {
  ...
  if (item) {
    ...
    if (category === 'dataStructure') {
      ...
      await fetchVideosByCategory(AlgorithmCategory.DATA_STRUCTURE);
      ...
      <div class="main-content">
        ...
        <div v-else class="video-display-area">
          <div class="display-header">
            <h2 class="selected-item">{{ getSelectedItemName() }}</h2>
            <p class="item-description">{{ getSelectedItemDescription() }}</p>
          </div>
